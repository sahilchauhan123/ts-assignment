...
func NewTorso(DatabaseUrl string, AuthToken string) *Torso {

	url := DatabaseUrl + "?authToken=" + AuthToken

	TorsoDb, err := sql.Open("libsql", url)
	if err != nil {
		panic(err)
	}
	...
	for _, table := range tables {
		_, err = TorsoDb.Exec(table.createQuery)
		if err != nil {
...
func (t *Torso) CreateCourse(name string) error {
	_, err := t.db.Exec("INSERT INTO courses (name) VALUES (?)", name)
	return err
}

func (t *Torso) GetCourses() ([]types.Course, error) {
	rows, err := t.db.Query("SELECT id, name FROM courses")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (t *Torso) CreateSubject(name string, courseID int) error {
	_, err := t.db.Exec("INSERT INTO subjects (name, course_id) VALUES (?, ?)", name, courseID)
	return err
}

func (t *Torso) GetSubjects(courseId int) ([]types.Subject, error) {
	rows, err := t.db.Query("SELECT id, name FROM subjects WHERE course_id = ?", courseId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (t *Torso) CreateLesson(name string, subjectID int) error {
	_, err := t.db.Exec("INSERT INTO lessons (name, subject_id) VALUES (?, ?)", name, subjectID)
	return err
}

func (t *Torso) GetLessons(subjectId int) ([]types.Lesson, error) {
	rows, err := t.db.Query("SELECT id, name, subject_id FROM lessons WHERE subject_id = ?", subjectId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (t *Torso) CreateStudent(name string, courseID int) error {
	_, err := t.db.Exec("INSERT INTO students (name, course_id) VALUES (?, ?)", name, courseID)
	return err
}

func (t *Torso) GetStudents() ([]types.Student, error) {
	rows, err := t.db.Query("SELECT id, name, course_id FROM students")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (t *Torso) CreateLessonCompletion(studentID int, lessonID int) error {
	_, err := t.db.Exec("INSERT INTO lesson_completions (student_id, lesson_id) VALUES (?, ?)", studentID, lessonID)
	return err
}

func (t *Torso) GetLessonsProgressOfStudent(studentID int) ([]types.SubjectProgress, error) {
	rows, err := t.db.Query(`
